...
class TestClientPaymentLink(ClientTestCase):
    ...
    def test_payment_link_create(self):
        ...
        url = f"{self.base_url}/fake_payment_link_id"
        responses.add(responses.PATCH, url, status=200, body=json.dumps(result),
                      match_querystring=True)
        self.assertEqual(self.client.payment_link.edit(
            'fake_payment_link_id', init), result)
    ...
    def test_payment_link_notifyBy(self):
        ...
        url = f"{self.base_url}/fake_payment_link_id/notify_by/email"

        responses.add(responses.POST, url, status=200, body=json.dumps(result),
                      match_querystring=True)
        self.assertEqual(self.client.payment_link.notifyBy(
            'fake_payment_link_id', medium='email'), result)
    ...
    def test_payment_all_fetch(self):
        ...
        responses.add(responses.GET, url, status=200, body=json.dumps(result),
                      match_querystring=True)
        self.assertEqual(self.client.payment_link.fetch(
            'fake_payment_link_id'), result)

    @responses.activate
    def test_payment_link_cancel(self):
        result = mock_file('cancel_payment_link')

        url = f"{self.base_url}/fake_payment_link_id/cancel"

        responses.add(responses.POST, url, status=200, body=json.dumps(result),
                      match_querystring=True)
        self.assertEqual(self.client.payment_link.cancel(
            'fake_payment_link_id'), result)
